...
\[
  \widehat{\alpha} = \mbox{exp}(\widehat{\mbox{log}(\alpha)}).
\]
...
In a Frequentist setting for log-normal data in stock assessment you'll often see what they call a "bias correction" term ($-\sigma/2$) added to the equation to make the estimated mean be correct on the real scale. For a good discussion on marginal means see Gory et al. (2020) "A class of generalized linear mixed models adjusted for marginal interpretability".
...
```{r}
## Bring in that nimble R call function again.
Rlamw <- function(x){
...
hist(smax[smax < 10]) ## Some extreme values to remove.
...
plot(density(smsy[smax < 10])) ## Some extreme values to remove.
...
plot(density(sgen[smax < 10])) ## Some extreme values to remove.
...
A challenge with bespoke Bayesian models is that it can be harder to do model checking. Luckily, the R packages `DHARMa` combined with nimble's posterior prediction capability makes this relatively easy.
