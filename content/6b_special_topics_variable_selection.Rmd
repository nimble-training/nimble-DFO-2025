...
\[
  \mu_i = \beta_0 + \sum_{k=1}^{15} \beta_x z_k x_{ik}.
\]
...
plot(samples_nimble[inds,'beta[2]'])
plot(samples_nimble[inds,'z[2]'])
```
...
- `beta[4]`, `beta[4]` and `beta[6]` are often included.
...
plot(samples_nimble_RJ[inds,'beta[2]'])
plot(samples_nimble_RJ[inds,'z[2]'])
```
