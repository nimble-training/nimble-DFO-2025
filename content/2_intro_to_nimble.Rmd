...
```{r}
fit.lm <- lm(logRS ~ I(spawners/1000), data = nadina)
logalpha.lm <- coef(fit.lm)[1]
E.lm <- 1/abs(coef(fit.lm)[2])
...
Order of the code doesn't matter. The model is exactly the same. Note that we also didn't define the mean explicitly, but put it within the distribution. NIMBLE will lift the node in the background making this model precisely the same as above but with different internal naming.
...
model <- nimbleModel(nadina_model_code_2,
                    constants = list(nobs = nrow(nadina), S = nadina$spawners/1000),
                    data = list(logRS = nadina$logRS),
                    inits = list(logalpha = 1.5, logE = log(10.5), sigma = 0.5))
