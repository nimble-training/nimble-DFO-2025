...
mod$getNodeNames(dataOnly = TRUE)
any(mod$isData(mod$getNodeNames()))
...
We consider spawners counts of sockeye salmon the Fraser River. Recruits are estimated as the spawners plus the catch from that broodyear. For sockeye, this means adding years 3-7 recruits. We will start with Early Stuart sockeye as the example but combine multiple stocks as we progress. 
...
Let's try a slice sampler, which is what JAGS often does when Gibbs sampling isn't available. This is slower but generally will mix better than a random walk.
...
It is common to place ***UPDATE HERE NEXT***

```{r}
sox <- sox %>% arrange(broodyear)
tdiff <- sox$broodyear - lag(sox$broodyear)
...
sox_model_AR1 <- nimbleCode({
  ...
  logE ~ dnorm(5, sd = 3)
  E <- exp(logE)
  rho ~ T(dnorm(0,1), -1, 1)  ## Correlation term.
  ...
  for( i in 2:nobs ){
    mean_logRS_t[i] <- mean_logRS[i] + (mean_logRS[i-1] - logRS[i-1])*rho^tdiff[i]
    logRS[i] ~ dnorm(mean_logRS_t[i], sd = sigma*sqrt(1-rho^(2*tdiff[i])))
  }
})

model <- nimbleModel(sox_model_AR1,
                    constants = list(nobs = nrow(sox), S = sox$spawners/spscale, tdiff = tdiff),
                    data = list(logRS = sox$logRS),
...
What does $\rho$ mean here? If it's about $-0.2$, then if the previous recruit/spawner ratio was larger than expected, then the next one would also be larger than expected. This could be related to productivity or just noise.
...
$$
As a result, we have a time varying productivity model written as $\mbox{log}(\alpha_t) = \mbox{log}(a_0)+ z_t \delta$. This is a mixture model!
...
plot(1:nrow(sox), out.sum[[1]][idx,"Mean"], type = "l",  ylab = "Posterior Mean log alpha", xlab = "brood year")
```
