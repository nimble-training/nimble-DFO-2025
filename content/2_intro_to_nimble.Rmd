...
The next step once we have defined a model and compile it into C++ is to build the MCMC. We first configure the MCMC. By default it will work through the stochastic nodes that aren't actually observed and assign samplers. We will then build the MCMC which builds the nimble functions required to run the MCMC based on the chosen samplers. This can then be run in R or C++. Once we run our MCMC we can access the sampled values with the mcmc object via `mvSamples`.
...
We consider spawners counts of sockeye salmon the Fraser River. Recruits are estimated as the spawners plus the catch from that brood year. For sockeye, this means adding years 3-7 recruits. We will start with Early Stuart sockeye as the example but combine multiple stocks as we progress. 
...
We will build a Ricker stock recruit model for these data. The parameters are $r = \mbox{log}(\alpha)$, where $\alpha$ is the maximum recruits per spawner, and $E$, the unfished equilibrium population size (carrying capicity). For brood year $i$, $S_i$ spawners generate $R_i$ recruits. Assuming combined process and observation noise $w_{i} \sim N(0,\sigma)$,
...
fit.nim <- optim(c(0,100,0), fn) ## Slow as model not compiled into C++
fit.nim$par 
```
...
Let's adapt this model to compute reference points. Using the Ricker equation, we can calculate three key reference points relying on Lambert $W$, function to solve explicitly.
...
We can actually use Lambert W in our nimble code by adding it to nimble via `nimbleRcall`. Control for the `nimbleRcall` will be passed to the R evaluator, which will be slower than compiled C++. See `nimbleExternalCall` to call externally compiled code directly.

We need to let nimble know the data type of input and output to expect from the function and then nimble can interact with the R function within the nimble code itself. Even after it's compiled in C++, nimble will call R separately to execute the `nimbleRcall` function. 
...
We have seen create model `nimbleModel` and `nimbleMCMC`. However, to understand and customize your model, it's best to actually configure and build your own MCMC. Here let's find out about what MCMC algorithms are assigned by default.
...
plot(samples[, "sigma"])  ## See mixing is poor early in the chain.
...
The Fraser sockey data that we loaded in has many different stocks associated with it. We may want to model them all together and make different assumptions about their relationships. We will start by assuming each stock is independent and fit a model with each stock having different parameters for the stock recruit relationship. We can run nearly the identical model as before but looping over each stock for the prior distributions. Because we are not always assured that the years of data match up for each stock, we will include a constant index to relate an observation to the stock. We will also assume a constant standard deviation across stocks.
...
As a result, the distribution of $\mbox{log}(R_t/S_t)$ is more complicated as the sum of two normally distributed random variables.
...
model <- nimbleModel(sox_model_AR1,
                    constants = list(nobs = nrow(sox), S = sox$spawners/spscale, deltat = deltat),
                    data = list(logRS = sox$logRS),
                    inits = list(logalpha = 1.5, delta = 0.25, logE = log(100), sigma = 0.5, rho = 0))
cmodel <- compileNimble(model)
...
$$
As a result, we have a time varying productivity model written as $\mbox{log}(\alpha_t) = \mbox{log}(a_0)+ z_t \delta$. This is a now mixture model.
