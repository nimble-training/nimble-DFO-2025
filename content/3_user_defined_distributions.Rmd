...
* This then reduces the latent nodes that need to be sampled by the length of the data.
...
Now let's make a function that takes some setup. Note that when we call this function with setup we need to call `run` directly when using the function. Note for convenience we will create a `nimbleList` output of our summary to make things look nicer.
...
Let's use a simple mixture model example. We will observe normally distributed data, with mean and standard deviation that comes from two, unobserved groups.
...
Now we will write out own distribution function to marginalize over $z$.
...
dnorm_mix <- nimbleFunction(
  run = function(x = double(), mean = double(1), sd = double(1), 
    ...
    ans <- p[1]*dnorm(x, mean[i], sd[i], log = FALSE) + p[2]*dnorm(x, mean[i], sd[i], log = FALSE)
...
Note that a HMM just like above is what Tang et al. (2021) "Identification of recruitment regime shifts with a hidden Markov stock-recruitment model" used to identify regime shifts in productivity for Atlantic cod. See this paper for a specific fisheries example. Also, the upcoming DFO paper by Wor et al. (2025) "Recommendations for Estimating and Detecting Time-Varying Spawner-Recruit Dynamics in Fish Populations".

Now we will marginalize over the latent states, just like the mixture model. For context, the algorithm we are going to code is called a 'forward algorithm' (https://en.wikipedia.org/wiki/Forward_algorithm). The idea is that we want to efficiently sum over all possible states and paths using essentially linear algebra.

Below, for each observation, we figure out the probability of currently being in a state, multiply that by the probability of transitioning to a new state and then compute the density of the observation in that state. We sum so that we again accumulate the probability of being in each state given the data and solve in a forward way through all observations.
